...
static ssize_t ncm_write(struct file *file, const char __user *buf, size_t count, loff_t *off) {
    ...
    // Check if the caller is a system server
    if (!is_system_server()) {
        NCM_LOGE("ncm_write failed: Caller is a non-system process with uid %u\n", current->cred->uid.val);
        return -EACCES; // Return access denied if not a system server
    }

    memset(intermediate_string, '\0', sizeof(intermediate_string)); // Clear the intermediate string

    // Attempt to copy data from user space, checking for errors
    size_t bytes_not_copied;
    bytes_not_copied = copy_from_user(intermediate_string, buf, sizeof(intermediate_string) - 1);
    if (bytes_not_copied) {
        pr_err("Failed to copy data from user space: %zu bytes not copied\n", bytes_not_copied);
        return -EFAULT; // Return an appropriate error code
    }

    // Convert the copied string to an integer
    intermediate_value = simple_strtol(intermediate_string, NULL, 10);
    if (intermediate_value > 0) {
        // Update timeout and flag if the value is positive
        update_intermediate_timeout(intermediate_value);
        update_intermediate_flag(intermediate_activated_flag);
        return strlen(intermediate_string); // Return the length of the string written
    }

    return intermediate_value; // Return the converted integer value
}
