...
static int zram_pin_backing_file(struct zram *zram)
{
    ...
    // Check the result of copy_to_user to handle any errors
    if (copy_to_user(buf, &set, sizeof(int))) {
        pr_err("%s: Failed to copy data to user space\n", __func__);  // Include function name for better debugging
        return -EFAULT;  // Return an appropriate error if the copy fails
    }

    // Call ioctl to pin the file
...
static void zram_handle_comp_page(struct work_struct *work)
{
	...
	size = zram_get_obj_size(zram, handle);
    ...
    if (unlikely(ret)) {
        pr_err("Decompression failed! err=%d, page=%lu, len=%u, addr=%p\n", ret, handle, size, src);
