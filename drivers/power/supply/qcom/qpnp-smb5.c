...
int request_VREG_L8(struct smb_charger *chg, bool enable)
{
    ...
    if (!chg->VREG_L8_2P95 && of_get_property(chg->dev->of_node, "VREG_L8_2P95-supply", NULL)) {
    ...
    if (enable) {
        if (chg->VREG_L8_2P95 && !regulator_is_enabled(chg->VREG_L8_2P95)) {
            ...
            if (rc < 0) {
                pr_err("Couldn't set VREG_L8_2P95 regulator output voltage rc=%d\n", rc);
                return rc; // Return error if voltage setting fails
            }
